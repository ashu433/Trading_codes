...
def sensex(quote_CE, quote_PE, strikes_sensex, xox):
    ...
    if len(index_CE) == 0 or len(index_PE) == 0:
        ...
        if 'date' in df_CE.columns:
            df_CE['date'] = df_CE['date'].apply(
                lambda x: pd.Timestamp(year=x[0], month=x[1], day=x[2], hour=x[3], minute=x[4]))
        ...
        for j in range(len(strikes)):
            ...
            if Symbol_index[i] == "SENSEX":
                ...
                if len(index_CE) == 0 or len(index_PE) == 0:
                    ...
                    to_datetime = datetime.datetime.now()
                    interval = "minute"
                    content_CE = kite.historical_data(ATM_Token_CE, from_datetime, to_datetime, interval,
                                                      continuous=False, oi=True)
                    content_PE = kite.historical_data(ATM_Token_PE, from_datetime, to_datetime, interval,
                                                      continuous=False, oi=True)

                    Content_CE = str(content_CE)
                    Content_PE = str(content_PE)

                    cleaned_contents_CE = Content_CE.replace("datetime.datetime", "").replace(
                        ", tzinfo=tzoffset(None, 19800)", "")
                    cleaned_contents_PE = Content_PE.replace("datetime.datetime", "").replace(
                        ", tzinfo=tzoffset(None, 19800)", "")
    ...
    for i in range(len(unique_symbol)):
        ...
        for k in range(len(strikes)):
            ...
            if len(index_CE) == 0 or len(index_PE) == 0:
                ...
                to_datetime = datetime.datetime.now()
                interval = "5minute"
                content_CE = kite.historical_data(ATM_Token_CE, from_datetime, to_datetime, interval, continuous=False,
                                                  oi=True)
                content_PE = kite.historical_data(ATM_Token_PE, from_datetime, to_datetime, interval, continuous=False,
                                                  oi=True)
...
def Future_index():
    ...
    for k in range(len(Instrument_list)):
        ...
        to_datetime = datetime.datetime.now()
        interval = "5minute"
        content_Futures = kite.historical_data(token_id, from_datetime, to_datetime, interval, continuous=False,
                                               oi=True)

        Content_Futures = str(content_Futures)

        cleaned_contents_Futures = Content_Futures.replace("datetime.datetime", "").replace(
            ", tzinfo=tzoffset(None, 19800)", "")
        ...
        if 'date' in df_Futures.columns:
            df_Futures['date'] = df_Futures['date'].apply(
                lambda x: pd.Timestamp(year=x[0], month=x[1], day=x[2], hour=x[3], minute=x[4]))
...
def daily_bhav_copy_download():
    ...
    today_latest = datetime.today()

    formatted_date_latest = today_latest.strftime("%Y%m%d")

    # URL of the zip file
    zip_file_url = f"https://nsearchives.nseindia.com/content/fo/BhavCopy_NSE_FO_0_0_0_{formatted_date_latest}_F_0000.csv.zip"
    ...
    if response.status_code == 200:
        ...
        new_bhav_copy=pd.read_csv(save_dir+final_file)
        list_segment=["IDF","STF"]
        list_segment_to_replace=["FUTIDX","FUTSTK"]
        drop_column_name_list=["TradDt","BizDt","Sgmt","Src","FinInstrmTp","FinInstrmId","ISIN","SctySrs","FininstrmActlXpryDt","StrkPric","OptnTp","FinInstrmNm","SsnId","NewBrdLotQty","Rmks","Rsvd1","Rsvd2","Rsvd3","Rsvd4"]
        result_df = pd.DataFrame()

        for i in range(len(list_segment)):
            df=new_bhav_copy[new_bhav_copy["FinInstrmTp"] == list_segment[i]]
            Segment=df["TckrSymb"].to_list()
            Segment=list(set(Segment))
            Segment.sort()
            for segm in Segment:
                new_df=df[df["TckrSymb"]==segm]
                new_df['XpryDt'] = pd.to_datetime(new_df['XpryDt'], format='%Y-%m-%d')
                new_df['XpryDt'] = new_df['XpryDt'].dt.strftime('%d-%b-%y')
                new_df = new_df.sort_values(by='XpryDt', ascending=True, key=pd.to_datetime)
                new_df.insert(0, 'INSTRUMENT', list_segment_to_replace[i])
                new_df.drop(columns=drop_column_name_list, inplace=True)
                new_df.rename(columns={'TckrSymb': 'SYMBOL', 'XpryDt': 'EXPIRY_DT'}, inplace=True)
                result_df = pd.concat([result_df, new_df], axis=0)

        result_df.reset_index(drop=True, inplace=True)
        result_df.to_csv(save_dir+final_file,index=False)
    else:
        print("Failed to download the zip file.")

#############################################################  OLD BHAV COPY DOWNLOAD MODULE  #################################################################
...
def Chainging_expiry_days():
    ...
    with open(file_path, 'r') as file:
        ...
        if expiry_date == New_Initial_date:
            ...
            date_obj = datetime.strptime(New_expiry_date, "%d-%b-%y")
            New_expiry_date = date_obj.strftime("%d-%b-%Y")
...
def prompt_Stocks_CP_data_collection(DAYS, symbol):
    ...
    from_datetime = datetime.datetime.now() - datetime.timedelta(
        days=DAYS)  # From last & days historical limit 100 days for 5 mins
    to_datetime = datetime.datetime.now()
...
def F_O_Stocks_closing_price():
    ...
    non_common_symbols = [symbol for symbol in unique_symbol if symbol not in old_symbol_list] + [symbol for symbol in
                                                                                                  old_symbol_list if
                                                                                                  symbol not in unique_symbol]
    common_symbols=common_symbols+non_common_symbols
    ...
    for symbol in common_symbols:
        ...
        from_datetime = datetime.datetime.now() - datetime.timedelta(
            days=1)  # From last & days historical limit 100 days for 5 mins
        to_datetime = datetime.datetime.now()

        interval = "minute"
        content = kite.historical_data(instrument_token, from_datetime, to_datetime, interval, continuous=False,
                                       oi=True)
        Content = str(content)
        cleaned_contents = Content.replace("datetime.datetime", "").replace(", tzinfo=tzoffset(None, 19800)", "")
        data = ast.literal_eval(cleaned_contents)
    ...
    for non_common_quotes in non_common_symbols:
        ...
        if non_common_quotes in old_symbol_list:
            print(f"Outgoing Name from the OLD F-O Stocks list: {non_common_quotes}")
            ...
            for kk in Index:
                stock_data_old.loc[kk,"Status"]="Removed"
                stock_data_old.loc[kk,"Date of Update"]=formatted_todays_date

            stock_data_old.to_csv(path_main + "Old_list_F_O_stocks.csv", index=False)

        elif non_common_quotes in unique_symbol:
            print(f"New incoming Name from the F-O Stocks list: {non_common_quotes}")
            today_date = datetime.today()
            formatted_todays_date = today_date.strftime('%d-%b-%Y')
            ...
            Reason=stock_data_old["Reason"].unique().tolist()
            ...
            dict_old_saving = {"INSTRUMENT": Instrument_list, "SYMBOL": old_symbol_list, "Status": Status_list,
                               "Date of Update": Todays_Date,"Reason":Reason}
...
def Sectors_indices_closing_price():
    ...
    Sectors_list = ["IT", "FMCG", "HEALTHCARE", "PHARMA", "AUTO", "REALTY", "METAL", "MEDIA", "OIL AND GAS",
                    "CONSR DURBL", "PSU BANK", "INDIA VIX"]
    ...
    for index_1_min in list_1_min:
        ...
        from_datetime = datetime.datetime.now() - datetime.timedelta(
            days=1)  # From last & days historical limit 60 days for 1 min
        to_datetime = datetime.datetime.now()
...
def Pre_market_data_collection(path, content_read):
    ...
    for i in range(len(content_read['data'])):
        ...
        Total_traded_volume_eqilibrim_qty.append(
            content_read['data'][i]['detail']['preOpenMarket']['totalTradedVolume'])
    ...
    Pre_market_data_1 = {"Date": Date, "Time": Time, "Advance": Advance, "Decline": Decline, "Unchanged": Unchanged,
                         "Total Traded Value in Crores": Total_traded_value_in_cr, "Total Market Cap": Total_market_cap,
                         "Total traded volume": Total_traded_volume
        , "Symbol": Symbol, "Opening Price Today": Opening_price_today, "Closing Price Previous Day": Closing_price,
                         "Change": Change, "Percentage Change": Percent_change,
                         "Final Setteled Quantity": Final_setteled_qty,
                         "Total Turnover in Crore": Total_turn_over_in_cr, "Market Captilization": Market_cap_in_cr,
                         "Year High": Year_high, "Year Low": Year_low}

    Pre_market_data_3 = {"ATO Buy Qty": ATO_Buy_Qty, "ATO Sell Qty": ATO_Sell_Qty,
                         "Equilibrium Volume": Total_traded_volume_eqilibrim_qty, "Total Sell Quantity": Total_sell_Qty,
