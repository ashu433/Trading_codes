...
output_file.to_csv(path_main+"India_Vix_Historical.csv",index=False)


for i in range(1,len(csv_files)):
    data_new=file_cleaning(csv_files[i])
    data_old=pd.read_csv(path_main+"India_Vix_Historical.csv")
    merged = pd.merge(data_old,data_new, how='outer', indicator=False)
    merged.to_csv(path_main+"India_Vix_Historical.csv",index=False)
